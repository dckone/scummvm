    ...
    while(!_vm->shouldQuit()) {
        ...
        while(_frm <= scene->endFrame && _cur_scene == oldscene && !_vm->shouldQuit()) {
            ...
            if(remainingMillis > 0) {
                if (remainingMillis > 15) { remainingMillis = 15; }
...
void GameMaddog::_scene_iso_pause(Scene *scene) {
    ...
    if (_had_pause) { checkPause = false; }
    if (_frm > scene->endFrame) { checkPause = false; }
    if (scene->dataParam1 <= 0) { checkPause = false; }
    if (checkPause) {
